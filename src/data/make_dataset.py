...
def train_val_split(data: pd.DataFrame,test_size:float,random_state:int) -> tuple[pd.DataFrame,pd.DataFrame]:
    ...
    dataset_logger.save_logs(msg=f'data is split into train split and shape {train_data.shape} and val split with shape {val_data.shape}',log_level='info')
    dataset_logger.save_logs(msg=f'The parameter values are {test_size} for test size and {random_state} from random state',log_level='info')
    return train_data,val_data

def save_data(data: pd.DataFrame,output_path: Path):
    data.to_csv(output_path,index=False)
    dataset_logger.save_logs(msg=f'{output_path.stem + output_path.suffix} data save sucsessfuly to output folder',log_level='info')

def read_params(input_file):
    try:
        with open(input_file) as f:
            params_files = safe_load(f)

    except FileExistsError as e:
        dataset_logger.save_logs(msg='parameter file not found, switching to default value for train test split',log_level='error')
        default_dict = {'test_size':0.25,'random_state':None}
        ...
        dataset_logger.save_logs(msg=f'Parameter files read succsessfully',log_level='info')
        #read the parameters from the parameters file
        test_size = params_files['make_dataset']['test_size']
        random_state = params_files['make_dataset']['random_state']
        return test_size, random_state
