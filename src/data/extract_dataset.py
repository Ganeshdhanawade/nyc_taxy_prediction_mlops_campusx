...
extract_logger = CustomLogger(log_filename='extract_dataset', logger_name=log_file_path)
...
def extract_zip_file(input_path: Path,output_path: Path):
    with ZipFile(file=input_path) as f:
        ...
        extract_logger.save_logs(msg=f'{input_file_name} extracted sucsessfuly at the target path',log_level='info')
